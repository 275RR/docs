# Using BB Editor App

!!! note "Quick Start"
    This is a Quick Start guide for using the BB Editor app. See **`BB-Editor-V3-main/Read Me.rtf`** for detailed processes and workflows as an editor.

    !!! note
        The BB Editor tabs are in the order of your workflow.

    !!! warning
        GitHub Pull latest changes BEFORE making any changes to Excel, Data, or Code. After all changes using Excel or BB Editor are done, Push changes to GitHub. The App will remind you.

</br>

### 1. Run BBeditor.bat

![BB Editor Run Image](../images/bat.png){ loading=lazy } </br>
!!! note
    Double-click on the **`Run BBeditor.bat`** file to launch the BB Editor tool.

</br>

### 2. Configure BB Editor's settings

![BB Editor Settings Image](../images/editorSettings.png){ loading=lazy }  
!!! note
    Follow the instructions to configure all of the BB Editor's settings pages. </br>
    *This only needs to be completed once.*

</br>

![BB Editor Github Settings Image](../images/editorSettingsGithub.png){ loading=lazy }  
!!! note
    Use SSH Format to connect to this repo on GitHub: </br>
    `git@github.com: <REPO_USERNAME>/<REPO_NAME>.git` </br>
    *You could upload this code under your own GitHub account.*

</br>

### 3. Pull from GitHub

![BB Editor Github Pull Image](../images/editorPull.png){ loading=lazy }  
!!! note
    Click `Pull from GitHub` on the "GitHub Pull" tab as the first step of your workflow each time.

</br>

### 4. Make BB edits in Excel

![Excel Image](../images/excel.png){ loading=lazy }  
!!! note
    Open the `Surnames and/or Villages Excel file` and make the needed changes. </br>
    Use "Email Viewer" tab in BB Editor to help. </br>
    Repeat until all changes are done. </br>
!!! warning
    IMPORTANT: Save and close Excel before the next step.

</br>

### 5. Convert Excel to webpages

![BB Editor Convert Image](../images/editorConvert.png){ loading=lazy }  
!!! note
    Click on the "Convert" tab, select which Excel files to convert to webpages, and click `Create Webpages`.

</br>

### 6. Backup edits

![BB Editor Backup Image](../images/editorBackup.png){ loading=lazy }  
!!! note
    Click on the "Backup" tab and click `Create Backup` to save a local copy of the project.

</br>

### 7. Upload new edits

![BB Editor Upload Image](../images/editorUpload.png){ loading=lazy }  
!!! note
    Click on the "Upload" tab, select which webpages to upload, and click `Upload to BB Website`.

</br>

### 8. Push to GitHub

![BB Editor Github Push Image](../images/editorPush.png){ loading=lazy }  
!!! note
    Click on the "GitHub Push" tab, enter a message about your changes, and click `Push to GitHub`.
